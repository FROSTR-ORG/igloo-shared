...
export async function createBrowserOnboardSponsorshipPackage(
  ...
  return {
    ...
    label,
    packageText: await encodeBfOnboardPackage(onboardPayload, input.password),
    preview: {
      label,
...
export async function createBrowserOnboardSponsorshipPackageFromBfshare(
  ...
  const source = await decodeBfSharePackage(sourcePackageText, input.sourcePackagePassword);
